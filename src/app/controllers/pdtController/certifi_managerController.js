   ...
   certifi_mananger(req, res, next){
        Certificate.find({'code': { $exists: true,}})
            .then(certificates => {
                ...
                res.render('phongdaotao/certifi_manager',{
    ...
    editcer(req, res, next){
        ...
        Certificate.findOne({ 'masv': masv })
            .then(certificates => res.render('phongdaotao/editwaitcer',{
    ...
    update(req, res, next){
        ...
        Promise.all([Student.updateOne({ 'masv': masv }, req.body), Point.updateOne({ 'masv': masv }, req.body), Certificate.updateOne({ 'masv': masv }, req.body)])
            .then(certificates => res.redirect('/phongdaotao/certifi_manager'))
            .catch(next);
    }
    ...
    detail(req, res, next){
        Student.findById(req.params.id)
            .then(students => res.render('/phongdaotao/waitcertifi_mananger',{
