...
class stu_managerController {
    ...
    store(req, res, next) {
        ...
        point.save(function (err) {
            if (err) console.log(err);
            else res.redirect('/admin/stu_manager');
        });
    }
    ...
    editstu(req, res, next) {
        ...
        Student.findOne({ 'masv': masv })
            .then(students => res.render('admin/editstu', {
    ...
    update(req, res, next) {
        ...
        // Student.updateOne({ 'masv': masv }, req.body)
            .then(students => res.redirect('/admin/stu_manager'))
            .catch(next);
    }
    ...
    detail(req, res, next) {
        Student.findById(req.params.id)
            .then(students => res.render('/admin/stu_manager', {
                students: mongooseToObject(students)
            }))
            .catch(next);
    }
